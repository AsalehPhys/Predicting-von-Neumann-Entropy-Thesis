...
# -----------------------------------------------------------
CONFIG = {
    'processed_dir': './processed_Ising_full',
    ...
    'num_epochs': 500,
    'random_seed': 42,
    'best_model_path': 'best_model_Ising_2_12_new.pth',
    'dropout_p': 0.4,
    'grad_clip': 1.0,
}
...
class PhysicalScaleAwareLoss(nn.Module):
    def __init__(self, physics_weight=0.0, eps=1e-10):
        super().__init__()
...
class ExperimentalGNN(nn.Module):
    ...
    def forward(self, data):
        ...
        node_features = torch.cat([
            x[:, self.feature_indices['position']].unsqueeze(-1),
            x[:, self.feature_indices['spin_probs']].unsqueeze(-1),
...
def main(rank, world_size):
    ...
    # Load dataset
    dataset = SpinDataset(root=CONFIG['processed_dir'])
    if len(dataset) == 0:
